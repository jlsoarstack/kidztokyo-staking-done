...
export class GemFarm extends GemFarmClient {
  ...
  async initFarmerWallet(farm: PublicKey) {
    ...
    toast(`initialized new farmer ${this.wallet.publicKey.toBase58()}`);
  ...
  async stakeWallet(farm: PublicKey) {
    ...
    toast(`begun staking for farmer ${this.wallet.publicKey.toBase58()}`);
  ...
  async unstakeWallet(farm: PublicKey) {
    ...
    toast(`ended staking for farmer ${this.wallet.publicKey.toBase58()}`);
  ...
  async claimWallet(
    ...
    toast(`claimed rewards for farmer ${this.wallet.publicKey.toBase58()}`);
  ...
  async flashDepositWallet(
    ...
    toast(`added extra NFT for farmer ${this.wallet.publicKey.toBase58()}`);
  ...
  async addToBankWhitelistWallet(
    ...
    toast(`${addressToWhitelist.toBase58()} added to whitelist`);
  ...
  async removeFromBankWhitelistWallet(
    ...
    toast(`${addressToRemove.toBase58()} removed from whitelist`);
